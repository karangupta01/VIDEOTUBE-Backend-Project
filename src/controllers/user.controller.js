...
const registerUser = asyncHandler( async () => {
    ...
    // return res
    // Access token expires in short durations and refresh token are for longer duration
  ...
  const existedUser = await User.findOne({
   ...
   const coverImageLocalPatrh = req.files?.coverImage[0]?.path;
   ...
   const coverImage = await uploadOnCloudinary(coverImageLocalPatrh)
...
const changeCurrentPassword = asyncHandler(async(req, res) => {
    ...
    if (!isPasswordCorrect) {
        throw new ApiError(400, "Inva1lid old password")
    }
